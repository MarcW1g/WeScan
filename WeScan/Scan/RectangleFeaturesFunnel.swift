...
final class RectangleFeaturesFunnel {
    ...
    /// A higher value means the auto scan is quicker, but the rectangle will be less accurate.
    /// On the other hand, the lower the value, the longer it'll take for the auto scan, but it'll be way more accurate
    ...
    private func updateRectangleMatches() {
        ...
        for (index, currentRect) in rectangles.enumerated() {
            for (index2, rect) in rectangles.enumerated() {
                if index2 > index && currentRect.matches(rect.rectangleFeature, withThreshold: matchingThreshold) {
