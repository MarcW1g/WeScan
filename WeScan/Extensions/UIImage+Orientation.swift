...
extension UIImage {
    ...
    func rotated(by rotationAngle: Measurement<UnitAngle>, options: RotationOptions = []) -> UIImage? {
        ...
        let image = renderer.image { renderContext in
            ...
            let xScale = options.contains(.flipOnVerticalAxis) ? -1.0 : 1.0
            let yScale = options.contains(.flipOnHorizontalAxis) ? 1.0 : -1.0
            renderContext.cgContext.scaleBy(x: CGFloat(xScale), y: CGFloat(yScale))
